...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/10 20:45:55 by ifreire-          #+#    #+#             */
/*   Updated: 2022/03/10 20:45:57 by ifreire-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (isline == 1)
	{
		...
		if (read_size <= 0)
			return (line);
		size = size + get_size_line(buffer[fd]);
		if (isline == 1 && read_size > 0)
			isline = check(buffer[fd]);
		line = next_line(line, buffer[fd], size);
	}
	return (line);
}
/*
